...
public class CDTFoldingPreferenceBlock implements ICFoldingPreferenceBlock {
	...
	private Text addTextField (Composite composite) {
		 ...
		 * valid. If not, turn the background of the text field red. Also
		 * there's a hidden label that says "Invalid Regular Expression!", that
		 * one will be turned visible too.
		 */
		t.addModifyListener(e -> {
			try {
				// Try to compile the regex. If it's invalid a
	...
	/**
	 * Find out whether the user wants to fold or highlight matches of regular
	 * expressions in source code.
	 */
	private void highlightOrFoldCheckBox (Composite parent) {
		String[] choices = new String[] { CDTFoldingConstants.COMBO_CHOICE_FOLD,
	 ...
	 * is to fold lines of source code. If the user wants to highlight something
	 * instead, the color choosers will be enabled again.
	 */
	...
	private void restoreCBTViewer () {
		...
		// All checked regexes that the user entered in the previous Eclipse
		// instance, checked or not
		// or-ed together at this point
		String checkedString = store.getString(CDTFoldingConstants.CHECKED_STRING_INPUT);

		// All regexes that the user entered in the previous Eclipse instance,
		// checked or not
		// or-ed together at this point
		...
		// Add regexes to the view, check all regexes that were checked
		// previously
		cbtViewer.setInput(new ArrayList<String>() {
			{
	...
	private void orRegexes () {
		...
		// note that we'll access this in
		// CDTFolderHighlighter.apply(ITextEditor, ProjectionViewer)!
		store.setValue(CDTFoldingConstants.CHECKED_STRING_INPUT, sb.toString());

		sb.delete(0, sb.length());

		/*
		 * Save non-checked regexes aswell, such that the user doesn't have to
		 * enter these again when opening another instance of Eclipse.
		 */
